...
/** Processes notifications for active state of the specified port. The
  * class exposes the port and port active observables, and returns
  * [[PortEvent]] notifications indicating when the port host, interface
  * name or active state have changed. The observable exposed by this class
  * does not filter errors, and will complete when the port is deleted.
  */
class PortTracker(val portId: UUID, context: Context)
    ...
    /** Builds the event with the current state of the port.
      */
