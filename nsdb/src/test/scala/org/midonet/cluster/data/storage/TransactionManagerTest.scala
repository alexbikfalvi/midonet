    ...
    feature("Transaction manager handles create operations") {
        scenario("The object does not exist") {
            ...
            manager.getOps should contain (opKey -> TxCreate(
                defaultDevice, ZoomChange.Direct.id))
        }
        ...
        scenario("The object exists but is deleted") {
            ...
            manager.getOps should contain (opKey -> TxUpdate(
                defaultDevice, 0, ZoomChange.Direct.id))
        }
        ...
        scenario("The object is created if it does not exist") {
            ...
            manager.getOps should contain (opKey -> TxCreate(
                notFoundDevice, ZoomChange.Direct.id))
        }
    }
    ...
    feature("Transaction manager handles update operations") {
        ...
        scenario("The object is created in the same transaction") {
            ...
            manager.getOps should contain only (opKey -> TxCreate(
                newDevice, ZoomChange.Direct.id))
        }
        ...
        scenario("The object is updated twice in the same transaction") {
            ...
            manager.getOps should contain only (opKey -> TxUpdate(
                newDevice, 0, ZoomChange.Direct.id))
