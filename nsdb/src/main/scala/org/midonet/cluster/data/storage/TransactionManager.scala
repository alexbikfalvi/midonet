...
import org.midonet.cluster.data.ZoomVersion.ZoomChange
...
abstract class TransactionManager(classes: ClassesMap, bindings: BindingsMap)
    ...
    // This is a transaction-local cache of the original snapshot for each
    // object that is being modified during the transaction. This allows at
    // the moment the transaction is committed to determine the changes to
    // each object.
    protected val snapshots = new mutable.HashMap[Key, ObjSnapshot]()
    ...
    private def cachedGet(clazz: Class[_], id: ObjId): Option[ObjSnapshot] = {
        ...
        cache.getOrElseUpdate(key, {
            val snapshot = getSnapshot(clazz, id).toBlocking.first()
