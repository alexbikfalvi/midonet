...
import org.midonet.benchmark.tables.ScalableStateTableManager.{EmptySubscriber, KeyValue, ProtectedSubscriber}
...
object ScalableStateTableManager {
    ...
    /**
      ...
      */
    private[benchmark] class ProtectedSubscriber[K, V](tableKey: Key,
...
private class ScalableStateTableManager[K, V](table: ScalableStateTable[K, V])
    ...
    : Unit = {
        ...
        val removeEntry = this.synchronized {
            ...
            val removeEntry = if (oldEntry eq null) {
                ...
                null
            } else if (oldEntry.version == ScalableStateTable.PersistentVersion ||
            ...
            table.statAddCallback(entry.key, entry.value)
            removeEntry
        }
    ...
    private def update(entries: util.Collection[String], ver: Long): Unit = {
        ...
        this.synchronized {
            ...
            while (entryIterator.hasNext) {
                ...
                if (nextEntry != null) {
                    table.statAddStore(nextEntry.key, nextEntry.value)
                    ...
                    if ((prevEntry eq null) ||
                        prevEntry.version == ScalableStateTable.PersistentVersion ||
                        (prevEntry.version < nextEntry.version &&
                         nextEntry.version != ScalableStateTable.PersistentVersion)) {
            ...
            computeRemovedEntries(storage = true)
    ...
    private def proxySnapshot(update: Notify.Update): Unit = {
        ...
        while (index < update.getEntriesCount) {
            ...
            if (entry ne null) {
                ...
                table.statAddProxy(entry.key, entry.value)
            }
            index += 1
        }
        ...
        if (update.getEnd) {
            ...
            // Compute the removed entries.
            computeRemovedEntries(storage = false)
    ...
    private def proxyRelative(update: Notify.Update): Unit = {
        ...
        while (index < update.getEntriesCount) {
            ...
            if (entry.hasValue) {
                ...
                table.statAddProxy(newEntry.key, newEntry.value)
                ...
                if (oldEntry eq null) {
                    ...
                    table.statRemoveProxy(oldEntry.key, oldEntry.value)
    ...
    /**
      ...
      * The call of this method must be synchronized.
      */
    private def computeRemovedEntries(storage: Boolean): Unit = {
        // Compute the removed entries.
        ...
        while (removeIterator.hasNext) {
            ...
            if (!updateCache.containsKey(oldEntry.getKey)) {
                ...
                if (storage) {
                    table.statRemoveStore(oldEntry.getKey, oldEntry.getValue.value)
                } else {
                    table.statRemoveProxy(oldEntry.getKey, oldEntry.getValue.value)
                }
