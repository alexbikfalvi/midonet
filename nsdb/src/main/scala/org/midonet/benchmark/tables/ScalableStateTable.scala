...
import org.midonet.benchmark.tables.ScalableStateTable.TableEntry
import org.midonet.benchmark.tables.ScalableStateTableManager.ProtectedSubscriber
import org.midonet.cluster.backend.Directory
import org.midonet.cluster.data.storage.{StateTable, StateTableEncoder}
...
object ScalableStateTable {
    ...
    /**
      ...
      *                number.
      */
    private[benchmark] case class TableEntry[K, V](key: K, value: V, version: Int)
...
trait ScalableStateTable[K, V] extends StateTable[K, V] with StateTableEncoder[K, V]
                               with Logging with StateTableMetrics[K, V] {
    ...
    protected[benchmark] def tableKey: Key

    protected[benchmark] def directory: Directory
    protected[benchmark] def connection: Observable[StorageConnectionState]
    protected[benchmark] def proxy: StateTableClient
    protected[benchmark] def metrics: StorageMetrics

    protected[benchmark] def nullValue: V
    ...
    /**
      ...
      * subscribers.
      */
    private[benchmark] def close(e: Throwable): Unit = sync.synchronized {
        if (e ne null)
    ...
    /**
      ...
      */
    private[benchmark] def encodeEntryWithVersion(entry: TableEntry[K, V]): String = {
    ...
    /**
      ...
      */
    private[benchmark] def encodeEntryPrefix(key: K, value: V): String = {
    ...
    /**
      ...
      */
    private[benchmark] def decodeEntry(path: String): TableEntry[K, V] = {
    ...
    /**
      ...
      */
    private[benchmark] def decodeEntry(entry: Notify.Entry): TableEntry[K, V] = {
        try {
    ...
    /**
      ...
      */
    @inline private[benchmark] def accessibleDecodeKey(kv: KeyValue): K = {
        decodeKey(kv)
    }
