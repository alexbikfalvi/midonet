...
final class BridgeMapper(bridgeId: UUID, implicit val vt: VirtualTopology)
    ...
    private lazy val macUpdatesSubscription = Observable
        .merge(macUpdatesSubject)
        .subscribe(makeAction1(onMacUpdate), makeAction1(onThrow))
    ...
    //            +---------------------------------+
    private lazy val timerSubscription = Observable.timer(
    ...
    //   +------------------------------------------------+          |
    //   |             +--------------+                              |
    // Obs[Obs[Port]]->| map(mapPort) |------------------------------+ merge
    //                 +--------------+                              |
    //   +-----------------------------------------------------------+
    //   |  +-----------------------+  +----------------+
    //   +->| filter(isBridgeReady) |->| map(mapDevice) |-> SimulationBridge
    //      +-----------------------+  +-------+--------+
    //   +-------------------------------------+
    //   |                       +------------------------+
    // Obs[Obs[MacTableUpdate]]->| subscribe(onMacUpdate) |
    //                           +------------------------+
    ...
    private def isBridgeReady(bridge: TopologyBridge): JBoolean = {
        ...
        log.debug("Bridge {} ready: {}", bridgeId, ready)
        ready
    }
    ...
    private def completeBridge(): Unit = {
        log.debug("Bridge {} deleted", bridgeId)
    ...
    private def mapBridge(br: TopologyBridge): TopologyBridge = {
        ...
        log.debug("Update for bridge {} ports {}", bridgeId, portIds)
    ...
    /**
     * An [[rx.Observable]] obsrever method that receives updates from the
     ...
     *                        +-------------+
     * Obs[MacTableUpdate] -> | onMacUpdate |
     *                        +-------------+
     */
    private def onMacUpdate(update: MacTableUpdate): Unit = {
    ...
    private def onMacExpirationTimer(count: JLong): Unit = {
        log.debug("Bridge {} MAC expiration timer {}", bridgeId, count)
    ...
    private def mapDevice(br: TopologyBridge): SimulationBridge = {
        log.debug("Processing and creating bridge {} device", bridgeId)
